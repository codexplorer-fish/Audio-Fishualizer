...
function savePreset(preset, presetName) {
    const pendingPresetStr = preset
    ...
    if (currentFlags == "0=None" || currentFlags == "") {
        updatedFlags = "0=" + presetName
    } else {
        const currentFlagsArr = currentFlags.split(",")
        currentFlagsArr.splice(addAt, 0, addAt + "=" + presetName)
...
function checkPresetsVersion(presetsStr) {
...
function init(){
    presets = checkPresetsVersion(presets)
